...
# Mediapipe Mapping:
  ...
  RIGHT_FOOT_INDEX = 32
    ...
    def obtain_LShoulderPitchRoll_angles(self, P1, P5, P6, P8):
        ...
        x = np.dot(v_8_1, np.cross(R_left_torso, v_5_6)) / (
                    np.linalg.norm(v_8_1) * np.linalg.norm(np.cross(R_left_torso, v_5_6)))
        try:
    ...
    def obtain_RShoulderPitchRoll_angles(self, P1, P2, P3, P8):
        ...
        x = np.dot(v_8_1, np.cross(R_right_torso, v_2_3)) / (
                    np.linalg.norm(v_8_1) * np.linalg.norm(np.cross(R_right_torso, v_2_3)))
        try:
